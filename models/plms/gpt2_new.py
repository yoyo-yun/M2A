...
from M2A.models.plms.LoRA import OurConv1D
...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.is_cross_attention:
            self.c_attn = OurConv1D(2 * self.embed_dim, self.embed_dim)
            self.q_attn = OurConv1D(self.embed_dim, self.embed_dim)
        else:
            self.c_attn = OurConv1D(3 * self.embed_dim, self.embed_dim)
        self.c_proj = OurConv1D(self.embed_dim, self.embed_dim)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            key, value = self.c_attn(encoder_hidden_states,
                                     nodes_hidden_states=nodes_hidden_states,).split(self.split_size, dim=2)
